...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Charter - (who is the community, what is the goal (mission statement))
% Terminology - Could be a reference to full section later in the document
% Standard Overview - Architecture (the roll of PMIx and introduction to the important software components (e.g. resource managers, etc)
% Required functionality? (What must be part of an implementation.. possibly leave to other WG?, originally was meant to more like MPI section that defines scope, e.g. debugger's in MPI, etc.  Refer to "use cases" as these sections get developed and how implementations may choose which they support)
...
The concept of a portable interface for process management predates \ac{PMIx}.
\ac{PMI} has been used for quite some time as a means of exchanging wireup information needed for inter-process communication.  Two versions (PMI-1 and PMI-2) have been released as part of the MPICH effort, with PMI-2 demonstrating better scaling properties than its PMI-1 predecessor.
\ac{PMIx} was intended to expand on the PMI effort,
and was not designed to be a radical shift away from the basic concepts introduced by
its predecessors.
\ac{PMIx} API's were designed to easily allow a \ac{PMIx} implementation to 
be capable of easily supporting both of the existing PMI-1 and PMI-2 \acp{API} with 
minimal effort.

Since its introduction, \ac{PMIx} has 
differentiated 
itself from earlier \ac{PMI} efforts in the following ways:

\begin{itemize}
\item \ac{PMIx} provides an extended version of the \ac{PMI} definitions specifically designed for better scalability.
\item The target user of \ac{PMIx} has grown from MPI libraries to other libraries
and languages for HPC.
\item A reference implementation (\ac{PRI}) of \ac{PMIx} has vastly increased adoption
\item With the aid of the \ac{PRI}, several implementations of \ac{PMIx} API's have become available leading to further adoption.
\end{itemize}

These differentiators have led to the creation of this document to formally specify the intended behavior of the \{PMIx} API's.

More information about the \ac{PMIx} specification process and affiliated projects can be found at the \ac{PMIx} web site: \url{https://pmix.org}
