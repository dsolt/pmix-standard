...
\added{ In this chapter we describe some common terms and conventions used throughout this document.}
The \ac{PMIx} Standard has adopted the widespread use of key-value \textit{attributes} to add flexibility to the functionality expressed in the existing \acp{API}. Accordingly, the \deleted{community} \added{ \ac{ASC} } has chosen to require that the definition of each standard \ac{API} include the passing of an array of attributes. These provide a means of customizing the behavior of the \ac{API} as future needs emerge without having to alter or create new variants of it. In addition, attributes provide a mechanism by which researchers can easily explore new approaches to a given operation without having to modify the \ac{API} itself.

The \ac{PMIx} \deleted{community} \added{ \ac{ASC} } has further adopted a policy that modification of existing released \acp{API} will only be permitted under extreme circumstances. In its effort to avoid introduction of any such backward incompatibility, the community has avoided the definitions of large numbers of \acp{API} that each focus on a narrow scope of functionality, and instead relied on the definition of fewer generic \acp{API} that include arrays of \deleted{directives} \added{ key-value attributes} for ``tuning'' the function's behavior. Thus, modifications to the PMIx standard increasingly consist of the definition of new attributes along with a description of the \acp{API} to which they relate and the expected behavior when used with those \acp{API}.
...
\deleted{In addition, the document refers to the following entities and process stages when describing use-cases or operations involving \ac{PMIx}:}

\added{The following terms and terminology is used throughout this document:}
\begin{itemize}
...
\item \declareterm{slot}\declareterm{slots}\emph{slot} refers to an allocated \deleted{entry for a process} \added{set of resources assigned to a process}. \acp{WLM} frequently allocate entire nodes to a \emph{session}, but can also be configured to define the maximum number of processes that can simultaneously be executed on each node. This often corresponds to the number of hardware \acp{PU} (typically cores, but can also be defined as hardware threads) on the node. However, the correlation between hardware \acp{PU} and slot allocations strictly depends upon system configuration.
\item \declareterm{application}\emph{application} refers to a single executable (binary, script, etc.) member of a \emph{job}. Applications consist of one or more \emph{processes}, either operating independently or \deleted{in parallel} \added{cooperatively} at any given time during their execution.
\item \declareterm{job}\emph{job} refers to a set of one or more \emph{applications} \added{or job-steps} executed as a single invocation by the user within a session. For example, ``\textit{mpiexec -n 1 app1 : -n 2 app2}'' is considered a single \ac{MPMD} job containing two applications.
...
\item \declareterm{resource manager}\emph{resource manager} is used in a generic sense to represent the subsystem that will host the \ac{PMIx} server library. \deleted{This could be a vendor's \ac{RM}, a programming library's \ac{RTE}, or some other agent.} \added{This could be a vendor-supplied resource manager or a third-party agent such as a programming model's runtime library.}
...
This document\deleted{borrows freely from}\added{ uses other terms commonly used in other standards and specifications with the same meaning} (most notably from the \ac{MPI} and OpenMP standards) \deleted{in its use of notation and conventions}in an attempt to reduce confusion. 
...
Users should not use the \textbf{\code{PMIX}}, \textbf{\code{PMIx}}, or \textbf{\code{pmix}} prefixes in their applications or libraries so as to avoid symbol conflicts with \deleted{current and later versions of the} \ac{PMIx}\deleted{standard and} implementations \deleted{such as the PRI}.
